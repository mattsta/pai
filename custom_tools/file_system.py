...
async def read_file(path: str) -> str:
    ...
    try:
        content = await asyncio.to_thread((WORKSPACE / path).read_text, encoding="utf-8")
...
async def list_directory(path: str = ".") -> str:
    ...
    try:
        ...
        result = {"status": "failure", "reason": f"Error listing directory '{path}': {e}"}
    return json.dumps(result, indent=2)
...
async def append_to_file(path: str, content: str) -> str:
    ...
    try:
        ...
        result = {"status": "success", "result": f"Content appended to '{path}' successfully."}
    except FileNotFoundError:
        result = {"status": "failure", "reason": f"File not found at '{path}'"}
    except Exception as e:
        result = {"status": "failure", "reason": f"Error appending to file '{path}': {e}"}
    return json.dumps(result, indent=2)
...
async def delete_file(path: str) -> str:
    ...
    try:
        ...
        if not is_file:
            result = {"status": "failure", "reason": f"Not a file or not found at '{path}'"}
        else:
            await asyncio.to_thread(file_path.unlink)
            result = {"status": "success", "result": f"File '{path}' deleted successfully."}
    except Exception as e:
...
async def delete_directory(path: str) -> str:
    ...
    if dir_path == WORKSPACE:
        result = {"status": "failure", "reason": "Cannot delete the root workspace directory."}
        return json.dumps(result, indent=2)

    try:
        is_dir = await asyncio.to_thread(dir_path.is_dir)
        if not is_dir:
            result = {"status": "failure", "reason": f"Not a directory or not found at '{path}'"}
        else:
            await asyncio.to_thread(shutil.rmtree, dir_path)
            result = {"status": "success", "result": f"Directory '{path}' deleted successfully."}
    except Exception as e:
        result = {"status": "failure", "reason": f"Error deleting directory '{path}': {e}"}
    return json.dumps(result, indent=2)
